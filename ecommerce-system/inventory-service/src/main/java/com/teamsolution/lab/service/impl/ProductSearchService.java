...
public class ProductSearchService {
  ...
  public PageImpl<ProductDocument> search(ProductFilterRequest request, Pageable pageable) {
    ...
    if (hasValue(request.getCategorySlug())) {
      boolQuery.filter(f -> f.term(t -> t.field("categorySlug").value(request.getCategorySlug())));
    }

    // Filter brand
    if (hasValue(request.getBrandSlug())) {
      boolQuery.filter(f -> f.term(t -> t.field("brandSlug").value(request.getBrandSlug())));
    }
                      ...
                      r.field("basePrice")
                          .gte(
                              request.getMinPrice() != null
                                  ? JsonData.of(request.getMinPrice())
                                  : null)
                          .lte(
                              request.getMaxPrice() != null
                                  ? JsonData.of(request.getMaxPrice())
                                  : null)));
    }
